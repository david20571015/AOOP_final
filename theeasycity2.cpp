...
//  query.exec("LOAD DATA INFILE 'C:/ProgramData/MySQL/MySQL Server 8.0/Uploads/city.csv' INTO TABLE TheEasyCity2 FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\r\n' IGNORE 1 ROWS");
    ...
    // st = "delete from TheEasyCity2 where City regexp '^[^";
    // ss >> t; // start char
    // st += t;
    // st += "]|[";
    // ss >> t; // end char
    ...
    // st = "delete from TheEasyCity2 where ( ";
    // ss >> t; // min lat
    // st += t;
    // st += " < Lat and Lat < ";
    // ss >> t; // max lat
    ...
    // query.exec("select * from TheEasyCity2");

    // if (!query.next())
    //     ans += "NULL";
    // else
    // {
    //     st = "update TheEasyCity2 set Lon = (@temp:=Lon), Lon = Lat, Lat = @temp where Id regexp '";
    //     ss >> t; // m
    //     st += t;
    //     st += "$'";
    //     query.exec(QString::fromStdString(st));
    //     query.exec("select round(sqrt(pow(max(Lat) - min(Lat), 2) + pow(max(Lon) - min(Lon), 2)), 4) as val from TheEasyCity2");
    //     query.next();
