...
string SymmetricPairs::solve(const string &s)
{
    ...
    if(t3 == "0")
    {
        st = "select * from(select * from (select x1, y1 from (select * from (select * from (select round(Lon, " + t3 + ") +1-1 as x1, round(Lat, " + t3 + ") as y1, Id as Id1 from CityList )as a join (select round(Lon, " + t3 + ") +1-1 as x2, round(Lat, " + t3;
